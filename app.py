...
def init_ee_enterprise():
    """Autenticação profissional com tratamento de erro de caractere JSON."""
    try:
        if EE_KEYS_STR:
            # O parâmetro strict=False ignora erros de escape como \n
            key_dict = json.loads(EE_KEYS_STR, strict=False)
...
def get_fast_geometry(mun_ids):
    """Gera geometria simplificada para evitar erro de buffer no servidor."""
    geoms = []
    for m_id in mun_ids:
...
with st.sidebar:
    ...
    else: st.warning("⚠️ Aguardando Autenticação...")
    ...
    if st.button("📍 CARREGAR ÁREA", type="primary", use_container_width=True):
        if selecao and connected:
            with st.spinner("Mapeando limites..."):
                ...
                # Conversão segura para objeto EE
            ...
            st.error("Erro: O sistema não está autenticado no Google Earth Engine.")
